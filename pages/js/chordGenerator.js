...
class ChordGenerator {
    constructor() {
        ...
        this.scaleFormula = { major: [0, 2, 4, 5, 7, 9, 11] };
    }

    generateProgression(key, progression) {
        return progression.split(/\s*-\s*/).map(notation => {
            const match = notation.trim().match(/^([b#]?\d+)([mM]?)$/);
            if (!match) return { symbol: '--', type: '' };
            const degree = match[1];
            const type = match[2] === 'm' ? 'm' : '';
            return { symbol: `${degree}${type}`, type };
        });
    }
}
